...
try{
  ...
  await internal.page.waitForTimeout(4500);
  ...
  await internal.page.waitForTimeout(2500);
  const gameShot=await service.capture(s.id,'game-ready');
  await fs.copyFile(path.resolve(gameShot.path),'artifacts/bg-legacy-ui/game-ready.png');
